...
function animate()
{
	...
	if(s && !player.canClimb && player.vy ==0)
	{
		player.canClimb = true;
		console.log(player.canClimb)
		//player.vy += player.climbHeight;
	}
	if(a)
	{
	...
	while(platform1.hitTestPoint(player.bottom()) && player.vy >=0 && !player.canClimb)
	{
		// if(player.canClimb){
		//if(!player.canClimb){
			player.y--;
	...
	while(platform1.hitTestPoint(player.left()) && player.vx <=0 && !player.canClimb)
	{
		player.x++;
		player.vx = 0;
	}
	while(platform1.hitTestPoint(player.right()) && player.vx >=0 && !player.canClimb)
	{
		player.x--;
		player.vx = 0;
	}
